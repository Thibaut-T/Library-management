          ...
          <>
            <div className="flex justify-center">
              <h1 className="text-2xl font-bold">
                {userToShow.userName} {userToShow.userLastName}
              </h1>
            </div>
            ...
            <div className="px-8 py-4">
              ...
              {userToShow.ownedBooks &&
                userToShow.ownedBooks.map((book) => (
            ...
            <div className="px-8 py-4">
              ...
              {/* Display friends items */}
              {userToShow.favoriteGenres &&
                userToShow.favoriteGenres.map((genre) => (
          ...
          <Modal
            ...
            items={
              modalInfo.category === "favoriteBook"
                ? userToShow.ownedBooks
                  ? userToShow.ownedBooks.map((book) => findName(book, books))
                  : []
                : modalInfo.category === "ownedBooks" &&
                  modalInfo.action === "Delete"
                ? userToShow.ownedBooks
                  ? userToShow.ownedBooks.map((book) => findName(book, books))
                  : []
                : modalInfo.category === "ownedBooks" &&
                  modalInfo.action === "Add"
                ? books.map((book) => book.name)
                : modalInfo.category === "friends" &&
                  modalInfo.action === "Delete"
                ? userToShow.friends
                ...
                ? users
                    .filter((user) =>
                      userToShow.friends
                        ? user.id !== userId &&
                          !userToShow.friends.includes(
                            `${user.userName} ${user.userLastName}`
                          )
                        : null
                    )
                    .map((user) => `${user.userName} ${user.userLastName}`)
                : modalInfo.category === "favoriteGenres" &&
                  modalInfo.action === "Delete"
                ? userToShow.favoriteGenres
                  ? userToShow.favoriteGenres.map((genre) =>
                      findName(genre, genres)
            ...
            onSubmit={(selectedValue, category, action) => {
              ...
              if (category === "ownedBooks") {
                ...
                const friendId = users.find(
                  (user) =>
                    `${user.userName} ${user.userLastName}` === selectedValue
                )?.id;
